...
Uwaga: przekierowanie używa dosłownej ścieżki (`redirect("/silownia/{$workout->id}")`), NIE
`redirect()->route('gym-workouts.show', ...)`. Nazwana trasa `gym-workouts.show` nie istnieje
jeszcze na etapie tego taska (powstaje dopiero w Task 6) — użycie `route()` w tym miejscu
rzuciłoby `RouteNotFoundException` i wywaliło `store()` w trakcie testu z Kroku 5. Gdy Task 6
zarejestruje `gym-workouts.show` pod dokładnie tym samym URI (`GET /silownia/{workout}`),
przekierowanie będzie trafiać we właściwą stronę bez żadnej dalszej zmiany w tym kontrolerze.
...
(Trasa `gym-workouts.show`, pod którą przekierowuje `store()`, powstaje w Task 6 — do tego
czasu URI `/silownia/{id}` prowadzi do 404, co jest oczekiwane w trakcie budowy milestone'a;
testy z Kroku 5 sprawdzają tylko nagłówek `Location`, nie faktyczne odwiedzenie tej trasy.)
...
## M3 — Status: UKOŃCZONY (2026-08-26)
...
3 ustalenia Istotne — wszystkie naprawione w jednej fali poprawek, zweryfikowane przez ponowny,
zakresowy przegląd (wszystkie potwierdzone jako naprawione, zero nowych regresji, w tym pełna,
ręczna weryfikacja logiki przepływu sterowania przepisanej kolejki offline). Finalna liczba
testów: **156/156**.

Naprawione w finalnym review:
- **Istotne:** kolejka offline (`gymOfflineQueue.js`) traciła bezgłośnie mutacje dodane w
  trakcie trwającego opróżniania kolejki (`drainQueue`) — naprawione przez ponowny odczyt
  aktualnej głowy kolejki przed każdym usunięciem elementu, zamiast operowania na zastygłej
  migawce.
- **Istotne:** trwale nieudana mutacja (np. przestarzałe/międzykontowe 404) blokowała kolejkę
  na zawsze, zamrażając wszystkie kolejne wpisy — dodano rozróżnienie błędów trwałych
  (odpowiedź HTTP 4xx — porzuć wpis) od przejściowych (brak odpowiedzi — ponów).
- **Istotne:** `CLAUDE.md` nie dokumentował lekcji z Task 6 (jak bezpiecznie sprawdzać
  właściciela zasobu przez relację bez własnego scope'a prowadzącą do modelu z
  `BelongsToUser`), a docblock `GymSet.php` opisywał naiwną, błędną formę tego sprawdzenia —
  oba poprawione; istotne dla M4, które będzie działać w kontekście kolejki (queue worker).
- **Drobne (dołączone do fali poprawek):** brak sprawdzenia `type==='gym'` na
  `show`/`finishForm`/`finish` (użytkownik mógł odwiedzić/zakończyć własny trening biegowy
  przez endpointy siłowni — tylko wpływ na siebie, bez wycieku międzyużytkownikowego);
  brak wizualnego wskaźnika „oczekuje na synchronizację” przy serii zapisanej tylko lokalnie;
  martwa zmienna w `ExerciseHistory`; nieużywany import w `Create.vue`; brak modyfikatora
  `.number` na polach liczbowych live-loggera; brak tiebreakera sortowania dla treningów
  z tą samą datą; brak jawnego rzutowania `(int)` przy porównaniu właściciela.

Świadomie odłożone (udokumentowane w planie, nie porzucone po cichu):
- Plany treningowe od AI, automatyczna progresja ciężarów i zastępowanie ćwiczeń ryzykownych
  bezpieczniejszymi wariantami przez AI — M3 tylko ostrzega wizualnie, decyzję podejmuje
  użytkownik ręcznie; pełna logika AI dojdzie w M4.
- `muscle_group` jako wolny tekst zamiast ograniczonej listy — realne ryzyko niespójności
  taksonomii słownika w dłuższej perspektywie, ale to praca funkcjonalna, nie poprawka.
- Rzadki przypadek brzegowy: dwa równoczesne wywołania `drainQueue()` (np. `onMounted` i
  zdarzenie `online` niemal jednocześnie) mogłyby po cichu zgubić jeden wpis bez ponowienia —
  ryzyko istniało już przed tą falą poprawek (w gorszej formie), nie jest tym, co poprawki 1/2
  miały naprawić (te dotyczyły współbieżnego *dodawania* podczas jednego opróżniania, nie
  współbieżnych *opróżnień*), i jest skrajnie mało prawdopodobne przy realnym użyciu (dwoje
  użytkowników, logowanie treningu z jednej karty na raz). Odnotowane, nie naprawiane teraz.

Gotowe do M4 — Trener AI.
